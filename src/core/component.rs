//! Entity/Component 数据协议、组件记录和销毁原因。
...
impl<T: Any + fmt::Debug> Component for T {}

/// 按具体类型下转型一份数据实例。
/// 参数要求对象类型为 `'static`（std 只在 `dyn Any + 'static` 上提供下转型）；
/// 借用本身可以是任意短生命周期。
pub fn downcast_component<'a, T: Any>(data: &'a (dyn Any + 'static)) -> Option<&'a T> {
    data.downcast_ref::<T>()
}

/// 一条 Component 记录：外层管理 owner 与稳定顺序，内层是完整业务数据。
/// 本轮不拆分为多个组件。
