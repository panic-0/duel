...
/// 提交由执行器的受控关联提交统一处理。
pub struct DamageDraft {
    pub source_id: Option<PlayerId>,
    pub target_id: PlayerId,
    pub amount: u64,
    consumption: Vec<ComponentId>,
    updates: Vec<(ComponentId, Box<dyn Any>)>,
}

impl fmt::Debug for DamageDraft {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("DamageDraft")
...
impl DamageDraft {
    ...
    pub fn consume_buff(&mut self, buff_id: ComponentId) {
        self.consumption.push(buff_id);
    }

    /// 声明本次提交成功时就地更新一份数据实例（保留身份，不产生销毁事实）。
    /// 多次更新同一实例按声明顺序应用，最后一次生效。
    pub fn update_component(&mut self, buff_id: ComponentId, data: Box<dyn Any>) {
        self.updates.push((buff_id, data));
    }

    pub(crate) fn take_consumption(&mut self) -> Vec<ComponentId> {
        std::mem::take(&mut self.consumption)
    }

    pub(crate) fn take_updates(&mut self) -> Vec<(ComponentId, Box<dyn Any>)> {
        std::mem::take(&mut self.updates)
    }
}

/// 伤害修改规则：读取业务数据、修正伤害草稿、声明消耗与声明关联更新。
/// 只调整参数，不执行子 Operation，不直接修改世界或 Component 数据；
/// 也不得批量处理所有实例而绕过候选级排序。
...
#[derive(Debug)]
pub struct Damage {
    pub context: DamageDraft,
}

impl Damage {
    pub fn new(source_id: Option<PlayerId>, target_id: PlayerId, amount: u64) -> Self {
        Self {
            context: DamageDraft::new(source_id, target_id, amount),
        }
    }
}
...
impl Operation for Damage {
    fn execute(
        ...
        if context.state().player(self.context.target_id).is_none() {
            return skipped();
        }
        // 参数修改窗口：候选 =（Priority, 响应主体稳定顺序, 规则注册顺序），
        // 前一个修改对后一个可见。
        let registry = context.resource::<DamageRules>();
        let mut candidates: Vec<(Priority, usize, usize, ReactionTarget)> = Vec::new();
        {
                ...
                for (rule_order, entry) in registry.entries.iter().enumerate() {
                    for subject in entry.rule.candidates(&self.context, &query) {
                        let subject_order = match subject {
                            ...
                            ReactionTarget::Standalone => entry.order,
                        };
        ...
        for (id, data) in self.context.take_updates() {
            changes = changes.update_component(id, data);
        }
